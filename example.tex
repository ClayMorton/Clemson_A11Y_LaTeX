...
guide says to use tables for data, not for layout. Clemson's reading-order guide gives two
patterns for text set side by side. Tabs read left to right, then the next line; columns read
top to bottom, then left to right. A \texttt{tabular} reads like tabs, row by row: with both
names in its first row and both universities in its second, a screen reader reads ``First
Author Second Author Clemson University Clemson University''. Each author in a
\texttt{minipage}, as below, reads like columns, and each name is read with its own
university. A \texttt{tabular} kept for layout gets \verb|\tagpdfsetup{table/tagging=false}|
inside a group; it is then not announced as a table, but it still reads row by row.
