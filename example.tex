...
%------------------------------------------------------------------------------
% pdftitle: doubled braces keep commas in one title.
\title[pdftitle={{Accessible LaTeX, quickly}}]{Accessible LaTeX, quickly}
% pdfauthor: commas between names.
...
\begin{enumerate}
  ...
  \item Build and check with \verb|make -f Makefile.a11y check|. A script cannot judge
    everything, so the report ends with six checks by hand: it lists every alt text and
    every spoken formula with its line for you to read back, then names the tags, the
    ...
    manual checks, which the six are adapted from.
\end{enumerate}
...
without it the alt text is the LaTeX source, with only \verb|\label|, \verb|\displaystyle|
and spacing commands taken out, and the check warns about the formula. Say the
formula as you would read it aloud in class and name every letter and symbol the page
shows, so that a listener could write it down. The class
...
use \texttt{align}:
\mathalt{a plus b, all squared, equals a squared plus two a b plus b squared, which equals
a squared plus b squared plus two a b}%
\begin{align}
  (a+b)^2 &= a^2 + 2ab + b^2 \\
          &= a^2 + b^2 + 2ab
\end{align}
Matrices, cases, sums and fractions come from \texttt{amsmath}, which the class loads:
\mathalt{capital A equals the two by two matrix with rows one, two and three, four. f of
x equals x squared if x is greater than or equal to zero, and minus x if x is less than
zero. The sum from k equals one to n of the fraction one over k squared is less than or
equal to pi squared over six}%
\begin{equation}
...
\mathalt{a equals b; c equals d for all d}%
...
\mathalt{a sub 1 plus a sub 2 plus a sub 3 plus a sub 4 plus a sub 5 plus a sub 6 plus a sub 7
plus a sub 8 plus a sub 9 plus a sub 10, plus b sub 1 plus b sub 2 plus b sub 3 plus b sub 4
plus b sub 5 plus b sub 6 plus b sub 7 plus b sub 8 plus b sub 9 plus b sub 10, equals s}%
\begin{multline*}
...
% and README (How to write great alt text) says how to word one.
...
alt text.
\mathalt{C H 4 plus two O 2 gives C O 2 plus two H 2 O}%
...
Input: the values \mathalt{a sub 1 through a sub n}$a_1,\dots,a_n$. Output: the largest of them.
\begin{algorithmic}[1]
  \State \mathalt{m gets a sub 1}$m \gets a_1$
  \For{\mathalt{i gets 2}$i \gets 2$ to \mathalt{n}$n$} \If{\mathalt{a sub i is greater than
    m}$a_i > m$} \State \mathalt{m gets a sub i}$m \gets a_i$ \EndIf \EndFor
  \State \Return \mathalt{m}$m$
...
% for a logo, which WCAG 2.1 1.4.5 counts as essential; words in a picture whose
% look is not essential are redrawn as text. The artifact sits in its own centered
