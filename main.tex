...
\begin{enumerate}
  ...
  \item Declare the header cells of every table before its \verb|\begin{tabular}|.
    Section~\ref{sec:tables} shows the eight layouts, the one caveat and the
...
``more''. Link text stays black here and the PDF viewer draws the underline from the
annotation's border style, so no link is marked by colour alone. \verb|\href| prints words
...
Write \verb|Section~\ref{sec:math}| rather than ``the section above'': the reference becomes a
link, and it stays correct when the text moves. Label anything to point at, and point at it
by name and number: see Section~\ref{sec:math} on page~\pageref{sec:math},
Section~\ref{sec:notation}, Figure~\ref{fig:lab} on page~\pageref{fig:lab},
Table~\ref{tab:headerrow}, Equation~\eqref{eq:gauss}, Algorithm~\ref{alg:max},
Appendix~\ref{app:example}, and a citation with a page number~\cite[p.~3]{knuth1984lp}.
\subsection{Lists}
...
\texttt{typewriter text} marks a file name and \textsc{small caps} an initialism. Colour
carries no meaning here: the text is black throughout, the chart uses one shade of grey, and
coloured ink would still need a contrast of 4.5:1 for text and 3:1 for a graphic.
\subsection{Code}
...
\section{Mathematics}
\label{sec:math}
Mathematics is written into the PDF twice: as MathML, which lets a MathCAT-class reader speak
the real structure of a formula, and as alt text, which is what most readers hear. Write
\verb|\mathalt{spoken text}| immediately before a formula to set that alt text; without it the
alt text is the bare LaTeX source, including any \verb|\label|. Acrobat and PAC check PDF/UA-1
and list the MathML spacing elements (\texttt{mspace}, \texttt{mtd}, \texttt{mrow},
\texttt{mtext}) as empty tags: that is the checker's limit, not a defect in the file. The
class option \verb|math=af| drops those elements and keeps only the attached MathML file,
which gives a checker-clean tree.
...
\eqref{eq:group-a} and~\eqref{eq:group-b} below; \texttt{gather} centres lines that do
...
% multline splits one long formula over lines. The numbered form makes tagpdf
% report "structure with label 0 is unknown rerun" on every run, so the starred
...
\mathalt{p plus q plus r plus s equals t}%
\begin{multline*} p + q + r \\ + s = t \end{multline*}
...
the LaTeX source unless \verb|\mathalt| precedes it, as it does for the first quantity below.
% mhchem: $\ce{^{14}C}$, the isotope form, makes tagpdf report "structure with
% label 36 has already been used", so it is not used here; write the mass number
% in words or as text. Everything else in \ce tags cleanly.
% Meets: ISO 32000-2:2020 14.8.6.3 (Formula, MathML); WCAG 2.1 1.1.1.
Water is $\ce{H2O}$ and the sulfate ion is $\ce{SO4^2-}$; carbon-14 is written out in words.
\mathalt{9.81 metres per second squared}%
Standard gravity is \qty{9.81}{\metre\per\second\squared}, room temperature is
\qty{25}{\celsius}, and \num{12345.678} is grouped by \verb|\num| rather than by hand. A state
is $\ket{0}$ and an inner product is $\braket{\psi|\phi}$. Write a range as two quantities,
\qty{10}{\celsius} to \qty{20}{\celsius}: \verb|\qtyrange| loses its numbers in the alt text.
...
\begin{lemma} The document title is that H1, so no section may claim it. \end{lemma}
...
% listed "Currently incompatible" and tags its caption as plain text, and an
% algorithmic body inside a figure aborts the build. This form gives a tagged
% Caption, a label that \ref reaches, and the steps as a real list.
% Meets: ISO 32000-2:2020 14.8.4.6 (Caption), 14.8.4.9 (L); WCAG 2.1 1.3.1.
...
Complex pictures follow in Section~\ref{sec:longdesc}.
...
% picture shows people, so Clemson's people pattern is given in the prose above.
...
Figure~\ref{fig:panels} uses \verb|trim| and \verb|clip| to show two halves of the picture above.
...
title in brackets: the long form is printed under the figure, and the short form goes into the list.

% \caption[short]{long}: the long form goes under the figure, the short into the
% list. Meets: Tagged PDF BPG 1.0.1 4.2.6 (Caption); ISO 32000-2:2020 14.8.4.6.
\begin{figure}\centering
...
\begin{figure}\centering
  \begin{tikzpicture}[alt={Flow diagram: a box labelled LaTeX source, an arrow to the right, and a box labelled tagged PDF.}]
...
wherever the picture can be redrawn; a logo is the usual exception. When the picture has to
stay, give it \verb|actualtext={...}|, the words themselves, and a reader hears the words. A
picture that carries no information at all gets \verb|artifact|, and is skipped.

% actualtext={...} gives a picture of words the words themselves; artifact marks
% a picture that carries nothing, so it is skipped. The artifact sits in its own
% centred line with paragraph tagging off: a paragraph holding only an artifact
% would be an empty tag. Meets: ISO 32000-2:2020 14.9.4 (ActualText), 14.8.2.2.
...
many as Aiken. Table~\ref{tab:chartdata} lists the same numbers.
...
\begin{table}\centering
  \caption[The numbers behind the visitor chart]{The numbers behind
    Figure~\ref{fig:chart}}\label{tab:chartdata}
...
\begin{figure}\centering
  \includegraphics[width=0.3\linewidth,alt={Photograph of the cable wall. Full description in the appendix Extended image descriptions.}]{example}
  \caption[The cable wall, described in an appendix]{The cable wall in the
    network store. Full description in Appendix~\ref{app:desc}}\label{fig:wall}
\end{figure}
...
\begin{description}
  ...
  \item[Colour] Colour is never the only signal. Figure~\ref{fig:chart} draws every bar in one
    shade and names each bar on the axis, so nothing depends on telling colours apart.
\end{description}
...
caption above the table, and keep notes outside it. The eight layouts below, and the one
caveat that follows them, cover the cases that come up in a thesis.
...
is not a table, so it is tagged as presentation and a screen reader reads it as plain text
rather than announcing rows and columns. Better still, lay such text out without a \texttt{tabular}.

% 8. A layout grid is not a data table. Do not use one; if a tabular only
% arranges things (an author block), tag it as presentation so no TH/TD is read.
% Keep table/tagging=presentation inside a group: it is a local switch and, left
% open, it would demote every later table. Paragraph tagging is off around it
% so the centring leaves no empty paragraph.
% Meets: ISO 32000-2:2020 14.8.4.8 (Table); WAI-ARIA 1.1 role=presentation.
\begingroup\tagpdfsetup{para/tagging=false}%
{\centering
...
\strong{Figure~\ref{fig:wall}:} A colour photograph of a storage wall in a network room. Grey
