...
annotation's border style, so no link is marked by color alone. \verb|\href| prints words
...
\begin{figure}\centering
  \includegraphics[width=0.5\linewidth,alt={Coiled gray network cables with blue connectors, hung in even rows on a beige pegboard wall.}]{example}
...
% centered line with paragraph tagging off: a paragraph holding only an artifact
...
\begin{description}
  ...
  \item[Color] Color is never the only signal. Figure~\ref{fig:chart} draws every bar in one
...
% so the centering leaves no empty paragraph.
...
\strong{Figure~\ref{fig:wall}:} A color photograph of a storage wall in a network room. Gray
